 ...
 * The list of clauses of the problem.
 */
vector<vector<int> > clausesNew;

/**
 * The occurrence list of positive appearances for each value in the clause set.
 */
vector<vector<vector<int>* > > positiveClausesNew;

/**
 * The occurrence list of negative appearances for each value in the clause set.
 */
vector<vector<vector<int>* > > negativeClausesNew;
 ...
 * The stack that tracks the current execution state (the backtrack stack).
 */
vector<int> modelStackNew;

/**
 * An index indicating which is the next literal from the stack to be propagated.
 */
int indexOfNextLiteralToPropagateNew;

/**
 * The current decision level of the DPLL algorithm.
 */
int decisionLevelNew;

/**
 * The activity (number of conflicts in which appears) for each positive literal.
 */
vector<double> positiveLiteralActivityNew;

/**
 * The activity (number of conflicts in which appears) for each negative literal.
 */
vector<double> negativeLiteralActivityNew;
...
void expansion() {
  ...
  cin >>  numVariablesNew >> numClausesNew;
  clausesNew.resize(numClausesNew);

  // Initialize positive and negative appearances
  positiveClausesNew.resize(numVariablesNew + 1);
  negativeClausesNew.resize(numVariablesNew + 1);

  // Read clauses
  for (int clause = 0; clause < numClausesNew; ++clause) {
    int literal;
    while (cin >> literal and literal != 0) {
      // add to the list of clauses
      clausesNew[clause].push_back(literal);

      // add to the list of positive-negative literals
      if (literal >0) {
        positiveClausesNew[var(literal)].push_back((vector<int>*) &clausesNew[clause]);
      }
      else {
        negativeClausesNew[var(literal)].push_back((vector<int> *) &clausesNew[clause]);
      }
    }
  }
  ...
  // Initialize the remaining necessary variables
    // model and backtrack stack
  modelNew.resize(numVariablesNew + 1, UNDEFINED);
  indexOfNextLiteralToPropagateNew = 0;
  decisionLevelNew = 0;
    // heuristic
  positiveLiteralActivityNew.resize(numVariablesNew + 1, 0.0);
  negativeLiteralActivityNew.resize(numVariablesNew + 1, 0.0);
  conflictsNew = 0;
    // statistics
  propagationsNew = 0;
  decisionsNew = 0;
...
void ReturnNegative() {
  ...
  for (int i = 0; i < clausesNew.size(); i++) { 
        for (int j = 0; j < clausesNew[i].size(); j++)
        {
          clausesNew[i][j]*=-1;
          cout << clausesNew[i][j]<< " ";
        }
...
void eliminar(){
  ...
  for ( int j = 0; j < clauses.size(); ++j )
  {
     ...
     {
        std::vector<int>::iterator iter = clausesNew[j].begin();
        std::vector<int>::iterator end = clausesNew[j].end();
        for ( ; iter != end; )
        {
           if ( *iter == clauses[j][k] )
           {
              iter = clauses[j].erase(iter);
           }
           else
           {
              ++iter;
...
bool propagateGivesConflict() {
  while (indexOfNextLiteralToPropagate < modelStack.size()) {
    ...
    for (int i = 0; i < clausesToPropagate.size(); ++i) {
      ...
      //traverse the clause
      for (int k = 0; not isSomeLiteralTrue and k < clause.size(); ++k) {
...
int main(int argc, char* argv[]) {
  ...
  cout<<" ingrese la cnf principal" <<endl;
  parseInput();
  cout<<" imprimir la cnf principal "<< endl;
  ImprimirCNFMain();
  cout<<" Ingrese la siguiente clausula "<< endl;
  expansion();
  cout<<" mostramos la segunda clausula negative "<<endl;
  ReturnNegative();
